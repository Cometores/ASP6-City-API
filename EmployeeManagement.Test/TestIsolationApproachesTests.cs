...
public class TestIsolationApproachesTests
{
    ...
    public async Task AttendCourseAsync_CourseAttended_SuggestedBonusMustBeCorrectlyBeRecalculated()
    {
        ...
        // get course from database - "Dealing with Customers 101" (Name of the course)
        var courseToAttend =
            await employeeManagementDataRepository.GetCourseAsync(Guid.Parse("844e14ce-c055-49e9-9610-855669c9859b"));

        // get existing employee - "Megan Jones"
        var internalEmployee =
            await employeeManagementDataRepository.GetInternalEmployeeAsync(
        ...
        var expectedSuggestedBonus =
            internalEmployee.YearsInService * (internalEmployee.AttendedCourses.Count + 1) * 100;
